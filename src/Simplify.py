...
def get_core(disc_matrix: DiscernibilityMatrix):
...
def greedy_cover(disc_matrix: DiscernibilityMatrix, verbose: bool=True) -> set[str]:
    """
    1. 将所有冲突对视为未覆盖
    2. 不断选能覆盖最多未覆盖对的属性
    3. 更新已覆盖对
    4. 重复直到无未覆盖对或无法进一步覆盖
    """
    conflict_pairs = disc_matrix.all_conflict_pairs()
    matrix = disc_matrix.matrix
    # attribute全集
    attributes = disc_matrix.decision_table.attributes
